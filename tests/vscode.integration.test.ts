...
describe('vscode integration', () => {
  ...
  test('should dispatch batch agents with default template and verify orchestration', async () => {
    ...
    const firstRequestContent = await readFile(
      path.join(messagesDir, batchRequestFiles[0]),
      'utf8',
    );
    expect(firstRequestContent).toContain('BATCH_TEST_1: First query');
    expect(firstRequestContent).not.toContain('{{userQuery}}');

    const secondRequestContent = await readFile(
      path.join(messagesDir, batchRequestFiles[1]),
      'utf8',
    );
    ...
    const orchestratorContent = await readFile(
      path.join(messagesDir, orchestratorFiles[0]),
      'utf8',
    );
